...
fetch("./json/produtos.json")
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    const alternarOpcoes = () => {
      if (containerOpcoes.classList.contains('abrir')) { //fecha o container se ele já estiver aberto
        ...
        containerOpcoes.style.height = containerOpcoes.scrollHeight + 'px' //container de opcoes ocupa o tamanho maximo dos elementos que estão dentro dele
      }
    }
    ...
    opcoes.forEach(opcao => {
      opcao.addEventListener('click', (evento) => {
        ...
        opcaoSelecionada.dataset.value = evento.target.dataset.value //muda o placeholder de "Localização" para a localização que está selecionada
        containerOpcoes.classList.remove('abrir')
        containerSelecionarPersonalizado.classList.remove('abrir')
